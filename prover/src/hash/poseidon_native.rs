...
impl NativePoseidonHasher {
    ...
    pub fn hash2(&self, a: Fr, b: Fr) -> Fr {
        let mut poseidon = Poseidon::new_with_preimage(&[a, b], &POSEIDON_PARAMS.constants_2);
        poseidon.hash()
    }

    /// Hash three field elements
    pub fn hash3(&self, a: Fr, b: Fr, c: Fr) -> Fr {
        let mut poseidon = Poseidon::new_with_preimage(&[a, b, c], &POSEIDON_PARAMS.constants_3);
        poseidon.hash()
    }

    /// Hash four field elements
    pub fn hash4(&self, a: Fr, b: Fr, c: Fr, d: Fr) -> Fr {
        let mut poseidon = Poseidon::new_with_preimage(&[a, b, c, d], &POSEIDON_PARAMS.constants_4);
        poseidon.hash()
    }
    ...
    pub fn hash5(&self, a: Fr, b: Fr, c: Fr, d: Fr, e: Fr) -> Fr {
        let mut poseidon =
            Poseidon::new_with_preimage(&[a, b, c, d, e], &POSEIDON_PARAMS.constants_5);
        poseidon.hash()
    }
