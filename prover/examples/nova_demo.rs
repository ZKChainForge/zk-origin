...
fn main() {
    ...
    use zk_origin::prover::nova_prover::{NovaLineageProver, NovaParams};
    use zk_origin::*;
    ...
    let params = NovaParams::setup(policy.compute_hash()).expect("Failed to setup Nova");
    ...
    prover
        .initialize(genesis_lineage, initial_counters)
    ...
    for i in 0..3 {
        ...
        let witness = witness_gen
            .generate_witness(&transition)
            .expect("Failed to generate witness");

        let start = Instant::now();
        prover.prove_step(&witness).expect("Failed to prove step");
