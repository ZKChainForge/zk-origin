...
use zk_origin::hash::{MerkleTree, PoseidonHasher};
...
fn bench_poseidon_hash(c: &mut Criterion) {
    ...
    c.bench_function("poseidon_hash_two", |b| {
        b.iter(|| hasher.hash_two(&input1, &input2))
    });
}
...
fn bench_poseidon_hash_five(c: &mut Criterion) {
    ...
    c.bench_function("poseidon_hash_five", |b| b.iter(|| hasher.hash(&inputs)));
}
...
fn bench_merkle_prove(c: &mut Criterion) {
    ...
    c.bench_function("merkle_prove", |b| b.iter(|| tree.prove(7).unwrap()));
}
...
fn bench_merkle_verify(c: &mut Criterion) {
    ...
    c.bench_function("merkle_verify", |b| b.iter(|| proof.verify()));
}
