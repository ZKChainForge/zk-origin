...
use zk_origin::{LineageProver, LineageVerifier, OriginClass, OriginPolicy, Transition};
...
fn bench_verify(c: &mut Criterion) {
    ...
    for num_steps in [1, 10, 50, 100].iter() {
        ...
        group.bench_with_input(
            ...
            &(proof, verifier),
            |b, (proof, verifier)| b.iter(|| verifier.verify(proof).unwrap()),
        );
    }
...
fn bench_verify_detailed(c: &mut Criterion) {
    ...
    c.bench_function("verify_detailed", |b| {
        b.iter(|| verifier.verify_detailed(&proof))
    });
}
...
fn bench_proof_deserialization(c: &mut Criterion) {
    ...
    c.bench_function("proof_from_json", |b| {
        b.iter(|| zk_origin::LineageProof::from_json(&json).unwrap())
    });
}
