...
function NUM_ORIGIN_CLASSES() { return 6; }
...
function MAX_GOVERNANCE_VOTES() { return 100; }
...
function getRateLimit(originClass) {
    ...
    else if (originClass == ORIGIN_CLASS_USER()) { return RATE_LIMIT_USER(); }
    else if (originClass == ORIGIN_CLASS_ADMIN()) { return RATE_LIMIT_ADMIN(); }
    else if (originClass == ORIGIN_CLASS_BRIDGE()) { return RATE_LIMIT_BRIDGE(); }
    else if (originClass == ORIGIN_CLASS_GOVERNANCE()) { return RATE_LIMIT_GOVERNANCE(); }
    else if (originClass == ORIGIN_CLASS_SYSTEM()) { return RATE_LIMIT_SYSTEM(); }
    else if (originClass == ORIGIN_CLASS_EMERGENCY()) { return RATE_LIMIT_EMERGENCY(); }
    else { return 0; }
}
