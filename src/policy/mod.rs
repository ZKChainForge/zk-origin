...
mod tests {
    ...
    fn test_default_policy() {
        ...
        assert!(policy
            .get(&(OriginClass::Genesis, OriginClass::User))
            .copied()
            .unwrap_or(false));
        assert!(policy
            .get(&(OriginClass::User, OriginClass::User))
            .copied()
            .unwrap_or(false));

        assert!(!policy
            .get(&(OriginClass::User, OriginClass::Admin))
            .copied()
            .unwrap_or(false));
    }

    #[test]
    fn test_policy_leaves() {
        let leaves = get_policy_leaves();
        assert!(!leaves.is_empty());
        assert_eq!(leaves.len(), 21); // Based on your policy
    }
}